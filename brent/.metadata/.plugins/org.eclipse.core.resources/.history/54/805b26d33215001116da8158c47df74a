/*
 * Transitive Closure for an un-weighted, directed graph.
 * 
 ...
 * 
 * The TC result will contain all the back edges, it will likely use close to twice the space as the original.
 * 
 */
		...
		for (int i = 1; i <= dimension; ++i ) {
			...
			for (int j = 1; j <= dimension; ++j) {
				if (i == j) {
					...
					System.out.print("\t" + 1); // All other weights are equal
				} else {
